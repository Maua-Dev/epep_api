...
### Seed DynamoDB
From the repo root, with `STAGE=TEST` in `.env`:
...
Os testes `test_*_repository_dynamo.py` estão comentados — **não rodam no CI**.

Para rodar localmente: suba o Docker, rode o seed, reative o teste e execute o pytest normalmente. Sem Dynamo up, o teste falha.
